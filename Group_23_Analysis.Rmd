...
$$\log\left(\frac{p_i}{1-p_i}\right) = \beta_0 + \sum_{j}^{16}\beta_j\mathbb{I}_{j}(x_{i}) + \beta_{17}\mathbb{I}_{\textrm{sellable}}(x_i) + \beta_{18}\mathbb{I}_{colours}(x_i) + \beta_{19}x_{i, depth} + \beta_{20}x_{i, height} + \beta_{21}x_{i, width}$$
where:
...
* $\mathbb{I}_{\textrm{sellable}}()$ is an indicator function returning 1 if the i<sup>th</sup> item is sellable online and 0 otherwise
* $\mathbb{I}_{\textrm{colours}}()$ is an indicator function returning 1 if the i<sup>th</sup> item is available in other colors and 0 otherwise
